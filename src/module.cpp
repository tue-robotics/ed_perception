...
boost::shared_ptr<Module> loadPerceptionModule(class_loader::ClassLoader* loader, const std::string& model_list_name)
{
    std::string module_name_ = "perception_module";

    loader->loadLibrary();
    std::vector<std::string> classes = loader->getAvailableClasses<Module>();
    ...
    boost::shared_ptr<Module> perception_mod = loader->createInstance<Module>(classes.front());
