...
class OduDBBuilder{
    ...
    void learnImage(std::string model_name, cv::Mat& image){
        ...
        if(process_file(image_ipl, images, model_name, true)){
...
//        std::cout << "[" << moduleName_ << "] " << "Saving the tree..." << std::endl;
    ...
    bool process_file(IplImage* image, std::vector<FeatureVector>& images_learn, std::string model_name, bool save_debug_img) {
