...
function [patterns n_positive n_negative] = load_scans( file, positive_label)
    ...
    for i=1:n_scans
        ...
        for j=1:n_clusters
            ...
            if ( c ~= 0 ) 
                ...
                features = [features(1:2) features(4:end)]; % take out distance to sensor feaure
                
                c_naive_s = scan(i).clusters(j).class_naive_s;
                c_naive_m = scan(i).clusters(j).class_naive_m;
                
                [ r n_features ] = size(features);
                pattern = [  id class label c_naive_s c_naive_m weight 0 features];
